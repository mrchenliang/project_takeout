const renderClientsOrdersPage = function(data) {
  ...
  data.forEach(element => {
    ...
    }" data-orderidstatus=${element.order_id}'>
  ...
  <input type="text" data-est-time placeholder="Wait Time">
  </div>
  <button class="ui secondary button"> Confirm </div>`
      : `<button class="ui secondary button"> Pick Up </div>`
  }</button>
</div>
</div>
</div>
  ...
  $(".confirm-completed").on("click", function() {
    ...
    const queryString = `/clients/2/orders/${orderId}`;
    $.ajax(queryString, { method: "POST" }).done(value => {
