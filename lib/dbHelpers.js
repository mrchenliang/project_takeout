const dbHelpers = {
  ...
  getAllOrders: function(db, restaurant_id) {
    ...
    return db
      .query(
        ...
        (SELECT
        orders.id as order_id, orders.user_id, orders.placed_at,
        ...
        GROUP BY orders.id) as orders_summary ON orders_summary.user_id = users.id;`,
        params
      )
      .then(result => {
        return JSON.parse(JSON.stringify(result.rows));
  ...
  getOrderDetailsById: function(db, id) {
    const params = [];
    params.push(id);
    return db
      .query(
  ...
  getUsersPendingOrderDetails: function(db, user_id) {
    ...
    return db
      .query(
        `
        SELECT users.name as user_name, users.id as user_id, restaurants.name as restaurant_name, restaurants.id as restaurant_id, menu_items.name as menu_item_name, menu_items.id as menu_item_id, order_details.notes as notes, order_details.quantity as quantity, order_details.price as price, orders.placed_at as placed_at, orders.status as status FROM orders
        JOIN users ON orders.user_id = users.id
        JOIN order_details ON order_details.order_id = orders.id
