const dbHelpers = {
  ...
  getUsersPendingOrderDetails: function(db, user_id) {
    return this.getUsersPendingOrder(db, user_id)
      .then(resultId => {
        ...
        return db
          .query(`SELECT order_details.*, menu_items.name FROM order_details JOIN menu_items on order_details.menu_item_id = menu_items.id WHERE order_id = $1;`, params)
  ...
  submitOrder: function(db, user_id) {
    return this.getUsersPendingOrder(db, user_id)
      .then(result => {
        ...
        params.push(new Date());
        params.push(order_id);
        return db.query(`UPDATE orders SET status = 'submitted', placed_at = $1 where id = $2 RETURNING *`, params)
          .then(result => {
            return result.rows[0];
          })
      })
  ...
  registerUser: function(db, name, email, phone, password) {
    ...
    return db.query(`INSERT INTO users (name, email, phone, password) VALUES($1, $2, $3, $4);`, params).then(result => {
      return result.rows[0]
    }).catch(e =>
      setImmediate(() => {
        throw e;
