const dbHelpers = {
  ...
  getMenuItems: function(db, restId) {
    ...
    params.push(restId);
    return db
      .query(`SELECT menu_items.*, categories.name as category_name FROM menu_items JOIN categories ON menu_items.category_id = categories.id WHERE restaurant_id = $1;`, params)
      .then(result => {
        return JSON.parse(JSON.stringify(result.rows));
      })
      .catch(e =>
